...
static esp_err_t app_identification_cb(identification::callback_type_t type, uint16_t endpoint_id, uint8_t effect_id, uint8_t effect_variant, void* priv_data) {
    ...
    app_driver_identify(static_cast<app_driver_handle_t>(priv_data), endpoint_id, type, effect_id, effect_variant);
    return ESP_OK;
}
...
static esp_err_t app_attribute_update_cb(attribute::callback_type_t type, uint16_t endpoint_id, uint32_t cluster_id, uint32_t attribute_id, esp_matter_attr_val_t* val, void* priv_data) {
    ...
    if (type == PRE_UPDATE) {
        /* Driver update */
        app_driver_handle_t driver_handle = static_cast<app_driver_handle_t>(priv_data);
        err = app_driver_attribute_update(driver_handle, endpoint_id, cluster_id, attribute_id, val);
        // Blink orange briefly on incoming Matter attribute updates
        status_led_blink_orange_once();
    }
