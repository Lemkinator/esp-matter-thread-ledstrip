...
static esp_err_t app_driver_light_set_mode(led_driver_handle_t handle, esp_matter_attr_val_t *val)
{
    ESP_LOGI(TAG, "LED set mode: %d", val->val.u8);
    return led_driver_set_mode(handle, val->val.u8);
}
...
esp_err_t app_driver_identify(app_driver_handle_t driver_handle, uint16_t endpoint_id, identification::callback_type_t type, uint8_t effect_id, uint8_t effect_variant)
{
    ...
    if (type == identification::START) {
        err = led_driver_identify_start(handle, endpoint_id, effect_id, effect_variant);
    } else if (type == identification::STOP) {
        err = led_driver_identify_stop(handle, endpoint_id, effect_id, effect_variant);
    }
    return err;
}
